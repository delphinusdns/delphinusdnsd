...
extern int		additional_wildcard(char *, int, struct rbtree *, char *, int, int, int *, ddDB *, uint32_t);
...
int 		reply_eui48(struct sreply *, int *, ddDB *);
int 		reply_eui64(struct sreply *, int *, ddDB *);
...
int 		reply_kx(struct sreply *, int *, ddDB *);
...
int		reply_svcb(struct sreply *, int *, ddDB *);
int		reply_https(struct sreply *, int *, ddDB *);
...
reply_a(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_A, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
/* 
 * REPLY_EUI48() - replies a DNS question (*q) on socket (so)
 *			based on reply_a()
 */

int
reply_eui48(struct sreply *sreply, int *sretlen, ddDB *db)
{
	char *reply = sreply->replybuf;
	struct dns_header *odh;
	uint16_t outlen = 0;
	...
	TAILQ_FOREACH(rrp, &rrset->rr_head, entries) {
		...
		answer->rdlength = htons(sizeof(struct eui48));			/* 12 bytes */

		memcpy((char *)&answer->rdata, 
			&((struct eui48 *)rrp->rdata)->eui48, 
			sizeof(struct eui48));
		...
		/* can we afford to write another header? if no truncate */
		if (outlen + sizeof(struct answer) > replysize) {
			NTOHS(odh->query);
			SET_DNS_TRUNCATION(odh);
			HTONS(odh->query);
			odh->answer = 0;
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_EUI48, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
		...
		if (outlen > origlen)
			odh->answer = htons(eui_count + retcount);	
...
/* 
 * REPLY_EUI64() - replies a DNS question (*q) on socket (so)
 *			based on reply_a()
 */

int
reply_eui64(struct sreply *sreply, int *sretlen, ddDB *db)
{
	char *reply = sreply->replybuf;
	struct dns_header *odh;
	uint16_t outlen = 0;
	...
	TAILQ_FOREACH(rrp, &rrset->rr_head, entries) {
		...
		answer->rdlength = htons(8);			/* 12 bytes */

		memcpy((char *)&answer->rdata, 
			(char *)&((struct eui64 *)rrp->rdata)->eui64, 
			sizeof(struct eui64));
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_EUI64, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
reply_nsec3param(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_NSEC3PARAM, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
reply_nsec3(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	*somelen = nextlen;
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_NSEC3, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
reply_zonemd(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_ZONEMD, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
reply_caa(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_CAA, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
reply_hinfo(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_HINFO, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
reply_rp(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_RP, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
reply_nsec(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_NSEC, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
reply_generic_dnskey(struct sreply *sreply, int *sretlen, ddDB *db, uint16_t rrtype)
{
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, rrtype, rbt, reply, replysize, outlen, &rrsig_count, q->aa, zonenumberx);
...
reply_ipseckey(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	TAILQ_FOREACH(rrp, &rrset->rr_head, entries) {
		...
		switch (((struct ipseckey *)rrp->rdata)->gwtype) {
		case 1:
			gwlen = 4;
			break;
		case 2:
			...
			break;
		case 3:
			gwlen = ((struct ipseckey *)rrp->rdata)->dnsnamelen;
			break;
		default:
			gwlen = 0;
			break;
		...
		if ((outlen + sizeof(struct answer) + gwlen + 3 + 
			((struct ipseckey *)rrp->rdata)->keylen) > replysize) {
			NTOHS(odh->query);
			SET_DNS_TRUNCATION(odh);
			HTONS(odh->query);
			odh->answer = 0;
		...
		answer->rdlength = htons(((struct ipseckey *)rrp->rdata)->keylen + 3 + gwlen);

		outlen += sizeof(struct answer); /* XXX ? */

		p = &reply[outlen];
		*p = ((struct ipseckey *)rrp->rdata)->precedence;
		p++;
		*p = ((struct ipseckey *)rrp->rdata)->gwtype;
		p++;
		*p = ((struct ipseckey *)rrp->rdata)->alg;
		...
		outlen += ((struct ipseckey *)rrp->rdata)->keylen;
	...
	odh->answer = htons(ipseckey_count);

	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		int tmplen = 0;
		int origlen = outlen;
	
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_IPSECKEY, rbt, reply, replysize, outlen, &rrsig_count, q->aa, zonenumberx);
		...
		if (outlen > origlen)
			odh->answer = htons(ipseckey_count + rrsig_count);	
...
reply_rrsig(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, -1, rbt, reply, replysize, outlen, &a_count, q->aa, zonenumberx);
	if (tmplen == 0) {
		/* we're forwarding and had no RRSIG return with -1 */
		if (q->aa != 1)
			return -1;
...
reply_aaaa(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_AAAA, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
/* 
 * REPLY_KX() - replies a DNS question (*q) on socket (so)
 *
 */

int
reply_kx(struct sreply *sreply, int *sretlen, ddDB *db)
{
	char *reply = sreply->replybuf;
	struct dns_header *odh;
	int kx_count;
	...
	TAILQ_FOREACH(rrp, &rrset->rr_head, entries) {
		...
		answer->rdlength = htons(sizeof(uint16_t) + ((struct kx *)rrp->rdata)->exchangelen);

		answer->kx_priority = htons(((struct kx *)rrp->rdata)->preference);
		memcpy((char *)&answer->exchange, (char *)((struct kx *)rrp->rdata)->exchange, ((struct kx *)rrp->rdata)->exchangelen);

		name = ((struct kx *)rrp->rdata)->exchange;
		namelen = ((struct kx *)rrp->rdata)->exchangelen;

		outlen += (12 + 2 + ((struct kx *)rrp->rdata)->exchangelen);

		/* can we afford to write another header? if no truncate */
		if ((outlen + 12 + 2 + ((struct kx *)rrp->rdata)->exchangelen) > replysize) {
			NTOHS(odh->query);
			SET_DNS_TRUNCATION(odh);
			HTONS(odh->query);
			odh->answer = 0;
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_KX, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
		...
		if (outlen > origlen)
			odh->answer = htons(kx_count + retcount);	
	...
	SLIST_FOREACH(ad0, &addishead, addis_entries) {
		...
		if (rbt0 != NULL && find_rr(rbt0, DNS_TYPE_AAAA) != NULL) {
			...
			if (dnssec && q->dnssecok && (rbt0->flags & RBT_DNSSEC)) {
				...
				tmplen = additional_rrsig(ad0->name, ad0->namelen, DNS_TYPE_AAAA, rbt0, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
		...
		if (rbt0 != NULL && find_rr(rbt0, DNS_TYPE_A) != NULL) {
			...
			if (dnssec && q->dnssecok && (rbt0->flags & RBT_DNSSEC)) {
				...
				tmplen = additional_rrsig(ad0->name, ad0->namelen, DNS_TYPE_A, rbt0, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
reply_mx(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_MX, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
reply_ns(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(rbt1->zone, rbt1->zonelen, DNS_TYPE_NS, rbt1, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
trunc:
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_CNAME, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
reply_ptr(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_PTR, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
reply_soa(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_SOA, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
/* 
 * REPLY_HTTPS() - replies a DNS question (*q) on socket (so)
 *
 */


int
reply_https(struct sreply *sreply, int *sretlen, ddDB *db)
{
	char *reply = sreply->replybuf;
	struct dns_header *odh;
	uint16_t outlen;
	...
	TAILQ_FOREACH(rrp, &rrset->rr_head, entries) {
		...
		outlen += ((struct https *)rrp->rdata)->targetlen;
		
		memcpy(p, ((struct https *)rrp->rdata)->param, ((struct https *)rrp->rdata)->paramlen);
		p += (((struct https *)rrp->rdata)->paramlen);
		outlen += (((struct https *)rrp->rdata)->paramlen);
	...
	odh->answer = htons(https_count);

	/* Add RRSIG reply_https */
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		int tmplen = 0;
		int origlen = outlen;
		int retcount;

		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_HTTPS, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
/* 
 * REPLY_SVCB() - replies a DNS question (*q) on socket (so)
 *
 */


int
reply_svcb(struct sreply *sreply, int *sretlen, ddDB *db)
{
	char *reply = sreply->replybuf;
	struct dns_header *odh;
	uint16_t outlen;
	...
	odh->answer = htons(svcb_count);

	/* Add RRSIG reply_svcb */
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		int tmplen = 0;
		int origlen = outlen;
		int retcount;

		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_SVCB, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
reply_txt(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_TXT, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
reply_loc(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_LOC, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
reply_tlsa(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_TLSA, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
reply_sshfp(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_SSHFP, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
reply_naptr(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_NAPTR, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
reply_srv(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, DNS_TYPE_SRV, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
reply_nxdomain(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(rbt->zone, rbt->zonelen, DNS_TYPE_SOA, rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
...
reply_any(struct sreply *sreply, int *sretlen, ddDB *db)
{
	 ...
	 */
	if ((istcp == DDD_IS_UDP) && tcpanyonly == 1) {
		NTOHS(odh->query);
		SET_DNS_TRUNCATION(odh);
		HTONS(odh->query);
		odh->answer = 0;
	...
	if (outlen == 0) {
		return (retlen);
	} else if ((istcp == DDD_IS_UDP) && (outlen == 65535)) {
		odh->answer = 0;
		odh->nsrr = 0;
		odh->additional = 0;
		outlen = rollback;
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		if (rbt->flags & RBT_WILDCARD) {
			...
			tmplen = additional_wildcard(q->hdr->name, q->hdr->namelen, authority, reply, replysize, outlen, &retcount, db, res->zonenumber);
			if (tmplen != 0) {
				outlen = tmplen;
				odh->nsrr = htons(retcount);	
			}
...
create_anyreply(struct sreply *sreply, char *reply, int rlen, int offset, int soa, uint32_t zonenumberx, uint compress)
{
	...
	if ((rrset = find_rr(rbt, DNS_TYPE_IPSECKEY)) != 0) {
		...
		TAILQ_FOREACH(rrp, &rrset->rr_head, entries) {
			...
			answer->type = htons(DNS_TYPE_IPSECKEY);
			answer->class = htons(DNS_CLASS_IN);
			...
			case 2:
				...
				break;
			case 3:
				tmplen = ((struct ipseckey *)rrp->rdata)->dnsnamelen;
				break;
			default:
				tmplen = 0;
				break;
			...
			answer->rdlength = htons(3 + tmplen + ((struct ipseckey *)rrp->rdata)->keylen);
			...
			if (tmplen) {
				memcpy(&reply[offset], 
					(char *)&((struct ipseckey *)rrp->rdata)->gateway,
					 tmplen);
				offset += tmplen;
			}
	...
	if ((rrset = find_rr(rbt, DNS_TYPE_KX)) != 0) {
		...
		TAILQ_FOREACH(rrp, &rrset->rr_head, entries) {
			...
			pack16(&reply[offset], htons(((struct kx *)rrp->rdata)->preference));
			offset += sizeof(uint16_t);

			if (offset + ((struct kx *)rrp->rdata)->exchangelen > rlen)
				goto truncate;

			memcpy((char *)&reply[offset], (char *)((struct kx *)rrp->rdata)->exchange, ((struct kx *)rrp->rdata)->exchangelen);

			offset += ((struct kx *)rrp->rdata)->exchangelen;

			if (compress) {
				if ((tmplen = compress_label((u_char*)reply, offset, ((struct kx *)rrp->rdata)->exchangelen)) > 0) {
					offset = tmplen;
				} 
			}
	...
	if ((rrset = find_rr(rbt, DNS_TYPE_HTTPS)) != 0) {
		...
		TAILQ_FOREACH(rrp, &rrset->rr_head, entries) {
			...
			answer->type = htons(DNS_TYPE_HTTPS);
			answer->class = htons(DNS_CLASS_IN);
			...
			if ((offset + 2 + ((struct https *)rrp->rdata)->targetlen + ((struct https *)rrp->rdata)->paramlen) > rlen)
				goto truncate;

			p = (char *)&answer->rdata;

			pack16(&reply[offset], htons(((struct https *)rrp->rdata)->priority));
			p += sizeof(uint16_t);
			offset += sizeof(uint16_t);
			
			memcpy(p, ((struct https *)rrp->rdata)->target, ((struct https *)rrp->rdata)->targetlen);
			p += (((struct https *)rrp->rdata)->targetlen);
			offset += (((struct https *)rrp->rdata)->targetlen);

			memcpy(p, ((struct https *)rrp->rdata)->param, ((struct https *)rrp->rdata)->paramlen);
			p += (((struct https *)rrp->rdata)->paramlen);
			offset += (((struct https *)rrp->rdata)->paramlen);

			answer->rdlength = htons(sizeof(uint16_t) + ((struct https *)rrp->rdata)->targetlen + ((struct https *)rrp->rdata)->paramlen);
	...
	if ((rrset = find_rr(rbt, DNS_TYPE_SVCB)) != 0) {
		...
		TAILQ_FOREACH(rrp, &rrset->rr_head, entries) {
			...
			answer->type = htons(DNS_TYPE_SVCB);
			answer->class = htons(DNS_CLASS_IN);
			...
			if ((offset + 2 + ((struct svcb *)rrp->rdata)->targetlen + ((struct svcb *)rrp->rdata)->paramlen) > rlen)
				goto truncate;

			p = (char *)&answer->rdata;

			pack16(&reply[offset], htons(((struct svcb *)rrp->rdata)->priority));
			p += sizeof(uint16_t);
			offset += sizeof(uint16_t);
			
			memcpy(p, ((struct svcb *)rrp->rdata)->target, ((struct svcb *)rrp->rdata)->targetlen);
			p += (((struct svcb *)rrp->rdata)->targetlen);
			offset += (((struct svcb *)rrp->rdata)->targetlen);

			memcpy(p, ((struct svcb *)rrp->rdata)->param, ((struct svcb *)rrp->rdata)->paramlen);
			p += (((struct svcb *)rrp->rdata)->paramlen);
			offset += (((struct svcb *)rrp->rdata)->paramlen);

			answer->rdlength = htons(sizeof(uint16_t) + ((struct svcb *)rrp->rdata)->targetlen + ((struct svcb *)rrp->rdata)->paramlen);
	...
	if ((rrset = find_rr(rbt, DNS_TYPE_EUI48)) != 0) {
		...
		TAILQ_FOREACH(rrp, &rrset->rr_head, entries) {
			...
			answer->type = htons(DNS_TYPE_EUI48);
			answer->class = htons(DNS_CLASS_IN);
	...
	if ((rrset = find_rr(rbt, DNS_TYPE_EUI64)) != 0) {
		...
		TAILQ_FOREACH(rrp, &rrset->rr_head, entries) {
			...
			answer->type = htons(DNS_TYPE_EUI64);
			answer->class = htons(DNS_CLASS_IN);
...
reply_generic(struct sreply *sreply, int *sretlen, ddDB *db)
{
	...
	if (dnssec && q->dnssecok && (rbt->flags & RBT_DNSSEC)) {
		...
		tmplen = additional_rrsig(q->hdr->name, q->hdr->namelen, ntohs(q->hdr->qtype), rbt, reply, replysize, outlen, &retcount, q->aa, zonenumberx);
